...
export class LeftAsideComponent implements OnInit {
  ...
  onChangeChecked(itemCategoria: any) {
    ...
    if (itemCategoria.isCheckedItem) {
      ...
      let tempListSelected = this.listaProductosSelectedDB.filter(function (obj) {
  ...
  private obtenerListaProductos(itemCategoria: any) {
    ...
    this.__ws.getListaProductosPublic(itemCategoria["id_categoria"])
      .subscribe(
        (res: any) => {
  ...
  private removeDuplicatesProducts() {
    ...
    this.listaProductosSelectedDB = this.listaProductosSelectedDB.filter((v, i, a) => a.findIndex(v2 => (JSON.stringify(v) === JSON.stringify(v2))) === i)
    // }
  }
