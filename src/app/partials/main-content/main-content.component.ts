...
export class MainContentComponent implements OnInit {
  ...
  obtnerlistaClientes() {
    ...
    this.__ws.getListaClientes("EC001")
      .subscribe(
        (res: any) => {
  ...
  setClienteSelectedVenta(arg0: string) {
    this.ClienteData = this.listaClientes.find(i => i['id_cliente'] === parseInt(arg0));
  }
  ...
  private listarProdcutos(idVenta: string) {
    ...
    this.__ws.getListaProductosByVentaId(idVenta)
      .subscribe(
        (res: any) => {
          if (res.length > 0) {
  ...
  onClickListenerAgregarProductoButton(cardItem: any) {//AGREGAR AL CARRITO DE VENTA

    cardItem["cantidad_producto"] = 1;
    this.listaProductosVenta.push(cardItem)

    this.listaProductosVenta = this.listaProductosVenta.filter((item, index, self) => self.indexOf(item) === index);
  ...
  obtenerPrecioSubTotal(cardItem: any) {
    ...
    let subTotal = Number(cardItem.precio_venta * cardItem["cantidad_producto"]).toFixed(2)
  ...
  obtenerTipoUnidad(cardItem: any): string {
    ...
    if (cardItem.tipo_unidad == 3)
      return "Und(s)"
    if (cardItem.tipo_unidad == 1)
      return "Kilo(s)"
    if (cardItem.tipo_unidad == 2)
      return "Metro(s)"
    else return ""
  }
  ...
  onGuardarVentaClickListener() {
    ...
    let dataVenta = {
      venta: { id_cliente: this.ClienteData.id_cliente, id_empleado: 1, alias_venta: this.dataBusquedaParams.aliasVenta },
      ventaProducto: dataVentaProducto
    }

    this.__ws.sendModelSave("venta", dataVenta)
      .subscribe(
        (res: any) => {
          if (res.id > 0) {
